...
const STATUSES = ["Todos", "Activa", "Inactiva", "Bloqueada"];
const PAGE_SIZE = 8;

export const Cards = () => {
  const { cards, loading, error, getCards, deleteCard } = useCardsStore();
  ...
  useEffect(() => {
    getCards().catch((err) => showError(err.response?.data?.message || "Error al cargar tarjetas"));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const filtered = cards.filter((card) => {
    const cardNumber = (card.cardNumber || "").toString();
    const accountNumber = (card.accountNumber || "").toString();
    const matchesSearch =
      cardNumber.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="p-4">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-main-blue">Tarjetas</h1>
          <p className="text-gray-500 text-sm">
            Gestiona los plásticos emitidos, revisa estados y crea nuevas tarjetas.
          </p>
        </div>
            ...
            <tbody>
              {loading ? (
                ...
                paginated.map((card, index) => (
                  <tr key={card._id || card.id || `card-${index}`} className="border-t hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-gray-800">{card.cardNumber}</td>
                    <td className="px-4 py-3 text-gray-600">{card.accountNumber}</td>
                    <td className="px-4 py-3 text-gray-600">{card.expirationDate}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => deleteCard(card._id).catch((err) => showError(err.response?.data?.message || "Error al eliminar tarjeta"))}
      ...
      <CardModal isOpen={showModal} onClose={() => setShowModal(false)} />
    </div>
  );
};